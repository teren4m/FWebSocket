...
module WebSocketServer =  
  ...
  let ping (ctrl : MailboxProcessor<Msg>) interval = async {
      let pingData = ServerUtil.makeFramePing
      while true do
          do! Async.Sleep interval
          ctrl.Post(Data pingData)            
      }

  let getTextByMask (mask: byte[]) = 
      Seq.mapi (fun i x -> x ^^^ mask.[i % mask.Length]) >> Seq.toArray >> System.Text.Encoding.ASCII.GetString
  ...
  let getText (bytes: byte[]) = 
      match getMask bytes with
      | Some mask -> 
          let lastIndex = int ((0x7Fuy &&& bytes.[1]) + 5uy)
          bytes.[6..lastIndex] |> getTextByMask mask
      | None -> 
  ...
  let runWorker (tcp : TcpClient) (ctrl : MailboxProcessor<Msg>) ct = 
      ...
      ignore <| startMailboxProcessor ct (fun (inbox : MailboxProcessor<byte array>) ->
  ...
  let start (ipAddress, port) = 
    let dispose = runRequestDispatcher (ipAddress, port)
